...
const testLikes: { recipeId: number; userId: string }[] = [{ recipeId: 1, userId: testUsers[0].googleId }];

axs.defaults.baseURL = `http://localhost:${PORT}/api/`;
axs.defaults.withCredentials = true;

beforeAll((done) => {
  loginUser(testUserProfiles[0]).then(() => done());
});

beforeEach((done) => {
  initTest().then(() => {
    userService
      .findOrCreate(testUserProfiles[0])
      //Add recipes
